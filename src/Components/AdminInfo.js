...
class AdminInfo extends React.Component {
    ...
    componentDidMount = async () => {
        ...
        if(this.state.userinfo === ""){
            response = await unprotected_api_call(admin_info_url, {"AppId": id}) 
                ...
                <div className='flex justify-center items-center flex-col w-auto mt-20  rounded'>
                    ...
                    {this.state.userinfo==="" ? (<>
                        ...
                        <div className='flex flex-col justify-center items-center h-1/2 w-full'>
                            {this.state.userinfo.MemberID === "" ? ( 
                                <>
                                    <div className='flex justify-around bg-purple-600 text-white w-1/2 items-center m-1 p-2'>
                                    <a target="_blank">
                                        <img src={"https://firebasestorage.googleapis.com/v0/b/mb-software-a20dc.appspot.com/o/election%2Fdefault_face.png?alt=media&token=1e1bf3ad-292d-492c-b0bc-ee08cbb48139"} alt="no image" className='h-[150px] w-[150px] rounded-full'/>
                                    </a>
                                    </div>
                                    ...
                                    <div className='flex justify-around bg-purple-100 text-purple w-1/2 items-center m-1 p-2'>
                                        ...
                                        <span><b>MEMBER ID:</b> {"KYC PENDING"}</span>
                                    </div>
                        ...
                        <div className='flex flex-col justify-center items-center h-1/2 w-full my-10'>
                            {this.state.extraData.CountIssued ? (
                                <div className='flex flex-col w-full items-center'>
                                    <b>Slip Issued</b>
                                    <div>{this.state.extraData.CountData.CountTime}</div>
                                    <div>Location: {this.state.extraData.CountData.Location}</div>
                                    <div>Agent: {this.state.extraData.CountData.Agent}</div>
                            ...
                            {this.state.extraData.BallotIssued ? (
                                <div className='flex flex-col w-full items-center mt-5'>
                                    <b>Ballot Issued</b>
                                    <div>{this.state.extraData.BallotData.BallotTime}</div>
                                    <div>Location: {this.state.extraData.BallotData.Location}</div>
                                    <div>Agent: {this.state.extraData.BallotData.Agent}</div>
